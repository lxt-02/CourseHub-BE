...
@Service
public record AddCourseLessonUseCase(CourseApplicationService courseApplicationService) {

    public ApiResponse<CourseResponse> execute(UUID courseId, UUID moduleId, AddLessonCommand command) {
        return ApiResponse.success("Course lesson added successfully", courseApplicationService.addLesson(courseId, moduleId, command));
    }
}
