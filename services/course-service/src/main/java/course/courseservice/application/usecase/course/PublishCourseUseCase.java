...
@Service
public record PublishCourseUseCase(CourseApplicationService courseApplicationService) {

    public ApiResponse<CourseResponse> execute(UUID id) {
        return ApiResponse.success("Course published successfully", courseApplicationService.publish(id));
    }
}
