...
@Service
public record GetCourseByIdUseCase(CourseUseCaseHandler CourseUseCaseHandler) {

    public ApiResponse<CourseResponse> execute(UUID id) {
        return ApiResponse.success("Course fetched successfully", CourseUseCaseHandler.getById(id));
    }
}
