...
@Service
public record ReturnCourseToDraftUseCase(CourseApplicationService courseApplicationService) {

    public ApiResponse<CourseResponse> execute(UUID id) {
        return ApiResponse.success("Course returned to draft successfully", courseApplicationService.returnToDraft(id));
    }
}
