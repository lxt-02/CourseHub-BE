...
@Service
public record MoveCourseModuleUseCase(CourseApplicationService courseApplicationService) {

    public ApiResponse<CourseResponse> execute(UUID courseId, UUID moduleId, MoveModuleCommand command) {
        return ApiResponse.success("Course module moved successfully", courseApplicationService.moveModule(courseId, moduleId, command));
    }
}
